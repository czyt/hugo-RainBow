...
面向长文阅读的 Hugo 主题。正文采用纸色、衬线字体和清晰的阅读层级；导航和控件采用轻磨砂材质，目录保持轻量无边框。支持浅色、深色和移动端。
...
目录支持两种样式：`classic`（默认）以细导轨连接标题层级；`tree` 使用刻度树形导航，以短横线区分层级、圆点跟随阅读位置，悬停时预览对应章节。两种样式均用主题强调色标记当前章节。顶部导航采用跟随鼠标的共享高亮气泡，离开后返回当前栏目；键盘和减少动态效果模式直接定位。
...
目录是否展开由 `TocOpen` 控制，宽屏侧边目录初始展开。书签形折叠按钮始终可见，支持键盘和触摸操作；减少动态效果模式下关闭位移动画。`TocStyle` 也可在文章 front matter 中覆盖，未知值回退为 `classic`。`hover` 模式下复制按钮随代码块悬停或键盘聚焦显示，触摸设备始终显示。

目录滚动条使用细滑块，桌面悬停或键盘操作时显示，触屏保留淡色提示；预留滚动条空间，避免目录文字左右跳动。
